-- Three days of free use with a server-enforced allowance of three recipe
-- imports per UTC day. The installation identity is linked into the account
-- identity at sign-in so changing from guest to account does not restart it.
create table if not exists public.daily_free_access (
  identity_key text primary key check (identity_key ~ '^(user|guest):[0-9a-f-]{36}$'),
  started_at timestamptz not null,
  usage_day date not null,
  imports_today smallint not null default 0 check (imports_today >= 0)
);

create table if not exists public.daily_free_access_guest_links (
  guest_key text primary key check (guest_key ~ '^guest:[0-9a-f-]{36}$'),
  user_key text not null check (user_key ~ '^user:[0-9a-f-]{36}$'),
  linked_at timestamptz not null default now()
);

alter table public.daily_free_access enable row level security;
alter table public.daily_free_access_guest_links enable row level security;
revoke all on public.daily_free_access, public.daily_free_access_guest_links from public, anon, authenticated;
...
create or replace function public.refund_daily_free_import(p_identity_key text, p_usage_day date)
returns void
language plpgsql
security definer
set search_path = public, pg_temp
...
declare
  ...
  v_linked_user_key text;
begin
  if p_identity_key !~ '^(user|guest):[0-9a-f-]{36}$' then
    raise exception 'Invalid free access identity';
  end if;

  if p_identity_key like 'guest:%' then
    select l.user_key into v_linked_user_key
      from public.daily_free_access_guest_links l where l.guest_key = p_identity_key;
    if found then v_identity_key := v_linked_user_key; end if;
  end if;

  update public.daily_free_access
     set imports_today = greatest(0, imports_today - 1)
   where identity_key = v_identity_key and usage_day = p_usage_day;
end;
$$;

revoke all on function public.refund_daily_free_import(text, date) from public, anon, authenticated;
grant execute on function public.refund_daily_free_import(text, date) to service_role;
