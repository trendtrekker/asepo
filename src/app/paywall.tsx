...
export default function Paywall() {
  ...
  const buy = async () => { if (!selected || busy) return; setBusy(true); try { if (await purchasePackage(selected)) enterApp(); } catch (e) { const m=e instanceof Error?e.message:'Could not complete the purchase'; if(!m.toLowerCase().includes('cancel')) toast.show(m); } finally { setBusy(false); } };
  const restore = async () => { if (busy) return; setBusy(true); try { const ok=await restorePurchases(); toast.show(ok?'Asepo Pro restored':'No active Asepo Pro purchase found'); if(ok) enterApp(); } catch(e) { toast.show(e instanceof Error?e.message:'Could not restore purchases'); } finally { setBusy(false); } };
  return <Screen><ScrollView contentContainerStyle={{paddingBottom:insets.bottom+24}} showsVerticalScrollIndicator={false}>
    ...
    <View style={{paddingHorizontal:20,paddingTop:20}}><View style={{backgroundColor:c.accentTint,borderRadius:16,padding:14,flexDirection:'row',alignItems:'center',gap:12}}><Text style={{fontSize:22}}>✦</Text><View style={{flex:1}}><Text style={{color:c.text,fontSize:15,fontWeight:'700'}}>Free for 3 imports or 3 days</Text><Text style={{color:c.textSec,fontSize:12.5,marginTop:2}}>{Math.max(0,importLimit-importsUsed)} imports and {freeAccessDaysLeft} days remaining · whichever comes first</Text></View></View>
      ...
      <Pressable accessibilityRole="button" disabled={busy} onPress={enterApp} style={{height:54,marginTop:14,borderRadius:18,borderWidth:1.5,borderColor:c.accent,backgroundColor:c.surface,alignItems:'center',justifyContent:'center'}}><Text style={{color:c.accent,fontSize:16,fontWeight:'800'}}>Continue free</Text></Pressable>
      <Text style={{color:c.textSec,fontSize:11.5,lineHeight:17,textAlign:'center',marginTop:7}}>Use up to 3 recipe imports for 3 days — whichever comes first. No payment required.</Text>
